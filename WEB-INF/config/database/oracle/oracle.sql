...
# ConfigModel
# #############
ConfigModel.addEntry = INSERT INTO jforum_config (config_id, config_name, config_value) VALUES (jforum_config_seq.nextval, ?, ?)
...
UserModel.selectAllByLimit = SELECT * FROM ( \
        SELECT user_email, user_id, user_posts, user_regdate, username, deleted, ROW_NUMBER() OVER(ORDER BY username) LINENUM  \
...
PostModel.selectAllByTopicByLimit = SELECT * FROM ( \
    SELECT p.post_id, topic_id, forum_id, p.user_id, post_time, poster_ip, enable_bbcode, \
...
#
SearchModel.cleanSearchResults = DELETE FROM jforum_search_results WHERE session_field = ? OR ((SYSDATE - time_field)*24) > 1.0
SearchModel.cleanSearchTopics = DELETE FROM jforum_search_topics WHERE session_field = ? OR ((SYSDATE - time_field)*24) > 1.0
