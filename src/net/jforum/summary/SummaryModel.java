 ...
 *         href="mailto:franklin@hp.com">franklin@hp.com</a>)
 * 
 */
public class SummaryModel extends Spammer {
	...
	public void sendPostsSummary(List recipients) throws Exception {
		...
		int daysBefore = Integer.parseInt(SystemGlobals
				.getValue(ConfigKeys.SUMMARY_DAYS_BEFORE));

		// New date "X" days before now, where "X" is the number set on the
		// variable daysBefore
		long dateBefore = Calendar.getInstance().getTimeInMillis()
				- (daysBefore * 1000 * 60 * 60 * 24);
		...
		// String url = forumLink + "/" +
		// SystemGlobals.getValue(ConfigKeys.SERVLET_EXTENSION);
		SimpleHash params = new SimpleHash();
		params.put("posts", posts);
		params.put("url", forumLink);

		String subject = SystemGlobals
				.getValue(ConfigKeys.MAIL_SUMMARY_SUBJECT);
		if (subject == null)
		...
		this.prepareMessage(recipients, params, subject, SystemGlobals
				.getValue(ConfigKeys.MAIL_SUMMARY_FILE));
